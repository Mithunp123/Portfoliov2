...
const DecayCard: React.FC<DecayCardProps> = ({
  ...
  return (
    <div className="content select-none pointer-events-none" style={{ width: `${width}px`, height: `${height}px` }} ref={svgRef}>
      <svg viewBox="-60 -75 720 900" preserveAspectRatio="xMidYMid slice" className="svg rounded-[1.5rem] overflow-hidden border border-white/15">
        <filter id="imgFilter">
          <feTurbulence
            type="turbulence"
      ...
      <div className="card-text pointer-events-auto">{children}</div>
    </div>
  );
};
